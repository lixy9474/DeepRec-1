  ...
  def restore(self, restored_tensors, unused_restored_shapes):
    ...
    with ops.colocate_with(self.handle_op):
      ...
      if self.var._init_data_source is not None:
        ...
        slot_dims = []
        for i in range(0, len(self.var._primary._slots_info)):
          slot_dims.append(0)
        for val in self.var._primary._slots_info.keys():
          slot_dims[val] = self.var._primary._slots_info[val]
        slot_dims_op = ops.convert_to_tensor(slot_dims, preferred_dtype=dtypes.int64)
        if self.var._is_primary:
          ...
          with ops.control_dependencies([self.var._primary.importer]):
            import_op = gen_kv_variable_ops.kv_resource_import_v2(
               restored_tensors[0],
               self.handle_op, self.var._primary._handle,
               variables._try_guard_against_uninitialized_dependencies(self.name, self.op.initial_value),
