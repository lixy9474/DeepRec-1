...
TEST(EmbeddingVariableTest, TestSSDIterator) {
  KVInterface<int64, float>* hashmap = new SSDHashKV<int64, float>(testing::TmpDir(), ev_allocator());
  ...
  int64* key_p;
  float* val_p;
  key_p = (int64*)malloc(sizeof(int64));
  val_p = (float*)malloc(sizeof(float)*126);
  for (it->SeekToFirst(); it->Valid(); it->Next()) {
    it->Key(key_p, sizeof(int64));
    int64 key = *key_p;
    it->Value(val_p, 126 * sizeof(float), 0);
    ASSERT_EQ(key, index);
    for (int i = 0; i < 126; i++)
