...
template <class K>
void HbmMultiTierFeatureDescriptor<TValue>::SetDefaultValues(
  ...
  if (init_cursor.size() > 0) {
    ...
    for (int emb_index = 0; emb_index < FeatureDescriptor<TValue>::slot_infos_.size(); emb_index++) {
      for (; it != init_cursor.cend(); ++it, ++i) {
        value_address[i] = GetEmbedding(value_ptrs[*it], 0);
        default_value_address[i] =
            FeatureDescriptor<TValue>::GetDefaultValuePtr(emb_index, keys[i]);
      }
      ...
      int block_dim = 128;
      int value_len = FeatureDescriptor<TValue>::slot_infos_[emb_index].default_value_len;
      TF_CHECK_OK(GpuLaunchKernel(
...
#define REGISTER_KERNELS(ktype, vtype)                                        \
  template void HbmMultiTierFeatureDescriptor<vtype>::SetDefaultValues(     \
