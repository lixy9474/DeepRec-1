...
class SSDKV : public KVInterface<K, V> {
 public:
  SSDKV(std::string path) {
    ...
    current_version = 0;
    current_offset = 0;
    buffer_size = 1 << 27;  // Write 16MB at once.
    buffer_cur = 0;
    open_file_count = 100;
    EmbFile* ef = new EmbFile(path_, current_version, buffer_size);
    emb_files.push_back(ef);
    total_app_count = 0;
  ...
  std::pair<int64, int64> LookupFile(K key) {
    auto iter = hash_map.find_wait_free(key);
    EmbPosition* posi = iter.second;
    std::pair<int64, int64> r(posi->version, posi->offset);
    return r;
  }
 ...
 private:
  void CheckBuffer() {
    if (buffer_cur * val_len + val_len > buffer_size) {
      ...
      if (emb_files[current_version]->app_count >= max_app_count) {
        ++current_version;
        current_offset = 0;
        emb_files.push_back(new EmbFile(path_, current_version, buffer_size));
      }
      TF_CHECK_OK(UpdateFlushStatus());
      buffer_cur = 0;
    }
  ...
  void SaveKV(K key, const ValuePtr<V>* value_ptr, bool is_compaction=false) {
    ...
    auto iter = hash_map.insert_lockless(
    ...
    if ((*(iter.first)).second != ep){
      ...
      __sync_bool_compare_and_swap(&((*(iter.first)).second), (*(iter.first)).second, ep);
      delete old_posi;
    }
  }

  void SaveKVForRestore(K key, ValuePtr<V>* value_ptr, bool is_compaction=false) {
  ...
  void SingleThreadDynamicCompaction() {
    ...
    if (hash_size * 3 / 2 < total_app_count || total_app_count - hash_size > 10000000) {
  ...
  class EmbFile {
    public:
     ...
     EmbFile(std::string path_, size_t ver, int64 buffer_size) {
       ...
       file_addr = (char *)mmap(nullptr, file_size, PROT_READ,
                       MAP_PRIVATE, fd, 0);
       CHECK(fs.good());
       app_count = 0;
       is_deleted = false;
