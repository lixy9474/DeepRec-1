...
class SSDHashKV : public KVInterface<K, V> {
 public:
  explicit SSDHashKV(std::string path, Allocator* alloc_)
  : current_version(0),
    current_offset(0),
    buffer_cur(0),
    alloc(alloc_),
 ...
 private:
  void CheckBuffer() {
    size_t curr_offset = buffer_cur * val_len;
    if (curr_offset + val_len > buffer_size) {
      emb_files[current_version]->Write(write_buffer, curr_offset);
  ...
  void SaveKV(K key, const ValuePtr<V>* value_ptr, bool is_compaction = false) {
    ...
    memcpy(write_buffer + curr_offset, (char*)value_ptr->GetPtr(),
           val_len);
