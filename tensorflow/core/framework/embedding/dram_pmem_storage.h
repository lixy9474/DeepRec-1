...
class DramPmemStorage : public MultiTierStorage<K, V> {
 public:
  DramPmemStorage(const StorageConfig& sc,
    ...
    //pmem_feat_desc_  = feat_desc.clone();
    //pmem_feat_desc.SetAllocator(
    //    experimental_pmem_allocator(sc.path, sc.size[0]));
    //ERROR!!!!!!!!!!!!
    pmem_ = new PmemLibpmemStorage<K, V>(sc, feat_desc);
    value_ptr_size_ =
  ...
  Status Get(K key, void** value_ptr) override {
    ...
    /*void* new_value_ptr = dram_->CreateValuePtr(size);
    if (s.ok()) {
      dram_feat_desc_->CopyData(
          pmem_feat_desc_,
  ...
  void Insert(K key, void* value_ptr) override {
    LOG(FATAL)<<"Unsupport Insert(K, void*) in DramPmemStorage.";
  }
 ...
 private:
  ...
  //NormalFeatureDescriptor<V>* pmem_feat_desc_ = nullptr;
  //NormalFeatureDescriptor<V>* dram_feat_desc_;
