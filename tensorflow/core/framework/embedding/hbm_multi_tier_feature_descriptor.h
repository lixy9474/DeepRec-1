...
namespace embedding {
template<class V>
class NormalFeatureDescriptor;
#if GOOGLE_CUDA
template<class V>
class HbmMultiTierFeatureDescriptor: public FeatureDescriptor<V> {
 public:
  HbmMultiTierFeatureDescriptor(
      ...
      : dram_alloc_bytes_(sizeof(V*)),
        ...
        FeatureDescriptor<V>(slot_num,
                             need_record_freq,
                             need_record_version) {
      FeatureDescriptor<V>::CreateFreqAndVersionDescriptor(&dram_alloc_bytes_);
    }
  ...
  void InitSlotInfo(int emb_index, int64 embedding_dim,
    ...
    if (is_compute_alloc_bytes) {
      FeatureDescriptor<V>::ComputeAllocBytes(&hbm_alloc_bytes_);
    }
 ...
 friend class NormalFeatureDescriptor<V>;
 protected:
